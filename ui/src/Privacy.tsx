...
/* ④ Domain separation — the core trick: one secret → two unlinkable hashes. */
export function DomainSeparation({
  ...
  return (
    <div className="card viz">
      <h3>왜 익명인데 중복은 막힐까? — 도메인 분리</h3>
      <div className="domain">
        <div className="secret-node">당신의 비밀<br /><code>0x••••••••</code><br /><span className="muted">당신만 앎</span></div>
        <div className="forks">
          <div className="fork">
            ...
            <div className="hash-node commit">
              ...
              <span className="muted">Merkle 트리에 들어감</span>
            </div>
          </div>
          <div className="fork">
            ...
            <div className="hash-node nullify">
              nullifier
              <code>{short(nullifier) || "—"}</code>
              <span className="muted">중복 제출 차단용</span>
            </div>
          </div>
        </div>
      </div>
      <p className="explain">
        같은 비밀에서 나왔지만 <b>서로 다른 도메인</b>으로 해싱돼서, 비밀을 모르면
        이 둘을 연결할 수 없습니다. 그래서 <b>익명(누군지 모름)</b>이면서도
        <b> 1인 1회(nullifier 중복 차단)</b>가 동시에 성립합니다.
      </p>
    </div>
  );
}

/* ② ZK ceremony — replaces the spinner during submission with the real steps. */
const CEREMONY = [
  "비밀값으로 commitment 생성",
  "Merkle 트리 멤버십 영지식 증명 생성",
  "nullifier 공개 (신원과 분리)",
  "온체인 기록 — 작성자 정보 없음",
];
...
/* ① Dual panel — what only you know vs what the whole chain sees. */
  ...
  return (
    <div className="dual">
      <div className="pane you">
        ...
        <Row k="자유의견" v={feedback || "—"} />
      </div>
      <div className="pane chain">
        <div className="pane-h">🌐 온체인 · 누구나 봄</div>
        <Row k="commitment" v={short(commitment) || "—"} mono />
        <Row k="nullifier" v={short(nullifier) || "—"} mono />
...
/* ③ Find your vote — the public state where you can't be found. */
export function FindYourVote({
  ...
  return (
    <div className="card viz">
      <h3>여기서 당신의 제출을 찾아보세요 (못 찾습니다)</h3>
      <div className="chainstate">
        <div className="cs-block">
          <div className="cs-h">commitment 트리 · {chain.treeSize}명 등록</div>
          <div className="cs-note">개별 잎(commitment)은 공개 상태에서 열거되지 않고, root만 보입니다.</div>
        ...
        <div className="cs-block">
          ...
          <div className="nullifier-grid">
            {chain.nullifiers.map((n) => (
              <code
                key={n}
                className={reveal && n === myNullifier ? "nf mine" : "nf"}
              >
                {short(n, 8, 4)}
                {reveal && n === myNullifier && <span className="you-tag"> ← 당신</span>}
              </code>
            ))}
            {chain.nullifiers.length === 0 && <span className="muted">아직 제출 없음</span>}
          </div>
        </div>
      </div>
      <p className="explain">
        당신의 commitment와 nullifier는 <b>같은 비밀</b>에서 나왔지만, 어느 commitment가
        어느 nullifier의 짝인지 <b>수학적으로 찾을 수 없습니다.</b>
      </p>
      {myNullifier && !reveal && (
        <button className="ghost" onClick={onReveal}>✨ 내 nullifier만 나에게 표시</button>
      )}
    </div>
  );
}
